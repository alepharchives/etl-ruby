...
class Formatter # :nodoc:
    ...
    def build_callsite_index
        ...
        @callsite_analyzer.analyzed_classes.each do |classname|
            @callsite_analyzer.analyzed_methods(classname).each do |methname|
                ...
                puts "unable to get defsite for #{methname} in class #{classname}" if defsite.nil?
                file, line = get_callsite_info(defsite)
                index[normalize_filename(file)][line] = 
    ...
    def get_callsite_info(defsite)
        file = (defsite.nil? ? 'unknown' : defsite.file)
        line = (defsite.nil? ? 'unknown' : defsite.line)        
        [ file, line ]
    end
        ...
        if @do_cross_references and 
           (rev_xref = reverse_cross_references_for(filename, lineno))
            refs = rev_xref.map do |classname, methodname, defsite, count|
                file, line = get_callsite_info(defsite)
                XRefHelper.new(file, line, classname, methodname, count)
